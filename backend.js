...
function updateProfilePic() {
    ...
    let data;
    if (localStorage.getItem("pfp")) data = JSON.parse(atob(localStorage.getItem("pfp")));
    loginLink.addEventListener('click', () => {
        if (window.parent) window.parent.location.href = window.parent.location.protocol + "//" + window.parent.location.host + "/settings.html";
        else window.location.href = window.location.protocol + "//" + window.location.host + "/settings.html";
    });
...
onAuthStateChanged(auth, async user => {
    if (user) {
        ...
        if (!data.pfp) {
            console.log("[Profile]: Setting values to default.");
